...
export default function AllOrdered(){
    ...
    useEffect(()=>{
        function getItem(){
        axios.get("http://localhost:8040/orderedItemRoute/get").then((res)=>{
            setItem(res.data);
    ...
    const deleteDataC = (e) =>{
      ...
      if(result == true){
          axios.delete(`http://localhost:8040/orderedItemRoute/delete/${e._id}`).then((res)=>{
          }).catch(e =>{
              alert(e)
          })
      }else{
     ...
     const addtoCompleted = (e) => {
        ...
        if(result == true){
            ...
            console.log(newItem)
            axios.post("http://localhost:8040/completedOrder/add",newItem).then(() => {
